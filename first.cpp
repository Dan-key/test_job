...
short getdata(char *buf, int start, int len){
    ...
    for (int i = last_byte; i <= int_bytes_start; ++i){
        u_int32_t value_in_mem= static_cast<u_int32_t>( static_cast<unsigned char>(*(buf+i)));
        value_in_mem = value_in_mem<<((int_bytes_start - i)*8);
        h = h+ value_in_mem; // put memory from buf into val
    }
...
void putdata(char *buf, int start, int len, short value){
    ...
    if( int_bytes_start - last_byte == 0){ // if we work only in one byte
        ...
        *(buf+last_byte) = static_cast<char>(final_byte);
        ...
        unsigned char first_byte_char =  *(buf+last_byte);
        ...
        *(buf + last_byte) =  static_cast<char>(first_byte_char); // storing right part

        unsigned char last_byte_char  = *(buf+int_bytes_start);
        //clearing bits for value part
        last_byte_char = last_byte_char<<num_of_bits2;
        last_byte_char = last_byte_char>>num_of_bits2;
        last_byte_char =last_byte_char|value_byte_last_part;
        *(buf + int_bytes_start) =  static_cast<char>(last_byte_char); // storing left part
    }else{ // takes 3 bytes to store
         // fisrt of all we need count how many bits value will take in first and last byte
        int num_of_bits1 = 8 - ((start - len+1)%8); // in first byte
        int num_of_bits2 = rem_bytes_start+1; // last first byte
        unsigned char first_byte_char =  *(buf+last_byte); // store first byte from buffer to make mask
        unsigned char last_byte_char  = *(buf+int_bytes_start); // store first byte from buffer to make mask
        ...
        *(buf + last_byte + 1) = static_cast<char>(value_byte_middle_part); // storing middle part, middle part doesn't need mask, because all its bits will be taken
    }
